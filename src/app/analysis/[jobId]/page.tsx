...
export default function AnalysisPage({
  ...
  const successCount = data.result?.individualAnalyses.filter((a) => a.analysis).length ?? 0;
  ...
  return (
    <Shell>
      ...
      {/* Low-data warning */}
      {isComplete && successCount > 0 && successCount < 5 && (
        <div className="mt-6 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-300">
          Analysis based on limited data ({successCount} reel{successCount > 1 ? "s" : ""}) — patterns may be less reliable.
        </div>
      )}
      ...
      {isComplete && data.result && (
        <div className="mt-8 space-y-8">
          ...
          <IndividualReels analyses={data.result.individualAnalyses} />
          <GuidedTour />
        </div>
      )}
    </Shell>
